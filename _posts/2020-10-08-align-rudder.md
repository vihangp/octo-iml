...
description: We present Align-RUDDER an algorithm which learns from as few as two demonstrations. It does this by aligning demonstrations and also speeds up learning by reducing the delay in reward.
date:   2020-10-08
...
<div class="align-images l-body">
    <div>
        <img class="img-fluid rounded z-depth-1" src="{{ site.baseurl }}/assets/img/align-rudder/align-demo.png">
    </div>
</div>
{: refdef}
...
<div class="align-images l-body">
    <div>
        <img class="img-fluid rounded z-depth-1" src="{{ site.baseurl }}/assets/img/align-rudder/fourrooms/demonstrations-raw.png">
    </div>
</div>
{: refdef}
...
A similarity matrix can for example be built via successor representation.
    ...
    <div class="align-images l-body">
        <div>
            <img class="img-fluid rounded z-depth-1" src="{{ site.baseurl }}/assets/img/align-rudder/fourrooms/events.png">
        </div>
    </div>
    {: refdef}
    ...
    {:refdef: style="text-align: center;"}
      <div class="align-images l-body">
        <div>
            <img class="img-fluid rounded z-depth-1" src="{{ site.baseurl }}/assets/img/align-rudder/fourrooms/demonstrations-clustered.png">
        </div>
    </div>
    {: refdef}
...
2. **Determine an alignment scoring scheme:** The alignment algorithm requires a scoring system to distinguish similar events from dissimilar events. A scoring matrix $$\unicode{x1D54A}$$ has entries $$\unicode{x1D564}_{i,j}$$ that give the score for aligning event $$i$$ with $$j$$.
    ...
    {:refdef: style="text-align: center;"}
    ![](assets/img/align-rudder/fourrooms/scoring-matrix.png)
    {: refdef}
...
4. **Compute the profile model and the PSSM:** From the alignment we get the following: (a) an MSA profile matrix with column-wise event frequencies and (b) a position-specific scoring matrix (PSSM) which is used for aligning new sequences to the profile.
    ...
    {:refdef: style="text-align: center;"}
    ![](assets/img/align-rudder/fourrooms/msa-profile-pssm.png)
    {: refdef}
...
alignment, and $$L$$ is the profile length. Note that $$L \geq T$$ and $$x_t \not= e_t$$, since gaps are present in the alignment. The redistributed reward is proportional to the difference of scores of two consecutive events based on the PSSM, i.e. the reward for $$R_{t+1}$$ is redistributed according to the following equation (where $$C$$ is just a normalization constant):
    ...
    {:refdef: style="text-align: center;"}
    ![](assets/img/align-rudder/fourrooms/redistribution-score.png)
    {: refdef}

    The figure below shows the final reward redistribution.

    {:refdef: style="text-align: center;"}
    ![](assets/img/align-rudder/fourrooms/redistribution.png)
    {: refdef}
...
{:refdef: style="text-align: center;"}
![](assets/img/align-rudder/results/fourrooms.png){:width="400px"} ![](assets/results/eightrooms.png){:width="400px"}
{: refdef}
...
![](assets/img/align-rudder/minecraft_alignment.png)
